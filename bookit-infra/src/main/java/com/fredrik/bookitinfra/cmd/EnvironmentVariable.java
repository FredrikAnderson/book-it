...
public class EnvironmentVariable implements DeploymentConfig {
    ...
    @Override
    public String getCommand(String appInstanceName, String env) {
        String replacedStages = value.replace("_stage_", env);
        if (replacedStages.contains(" ")) {
            replacedStages = getStartEndArgumentTag() + replacedStages + getStartEndArgumentTag();
        }
        return String.format(cmd, appInstanceName, name, replacedStages);            
    }
